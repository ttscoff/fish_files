if functions -q expand-word
    ...
    function __get_history -d 'Get a specific entry from the command history.'
    ...
    function __split_history -d 'Split a history entry into tokens and print the specified one.'
    ...
    function __search_history -d 'Search the command history for a matching entry.'
        set -l search (string match -r '!(\w+)' (commandline -t))
        history search --contains "$search[2]" --max=15 | head -n 1
    end

    function __substitute_history -d 'Substitute text in a history entry using sed.'
    ...
    function __substitute_history_caret -d 'Substitute text in a history entry using ^ as delimiter.'
        set -l token (commandline -t)
    ...
    function __expand_path -d 'Expand a path token using ffdir, fffile, or fasd.'
        set -l token (commandline -t)
