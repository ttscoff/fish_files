if functions -q expand-word
	expand-word --pattern '^\d*s/..*/.*$' --expander '__substitute_history'
	expand-word --pattern '^\d*\^..*\^.*$' --expander '__substitute_history_caret'
	expand-word --pattern '^!\d+(?:\.(\d+))?' --expander '__get_history'
	expand-word --pattern '^![^0-9]\w+' --expander '__search_history'

	# expand-word --pattern '^//.*/$' --expander '__expand_path'
	# expand-word --pattern '^//.*[^/]$' --expander '__expand_path'
	expand-word --pattern '^//.*$' --expander '__expand_path'
	...
	function __search_history
		...
		history search --contains "$search[2]" | fzf -1 -0
